...
def try_decrypt(private_key, buffer, raise_on_error=False):
    ...
    try:
        ...
        compressed_messsage = unpad(
            aes_cipher.decrypt(ciphertext), block_size=AES_BLOCK_SIZE
        )
...
def queue_message(host_ip, payload, queue, public_key, is_public_key=False):
    ...
    if is_public_key:
        ...
        # Make the key chunks 160 bits by appending 128 bit chunks with 32 bits of
        cid_payloads = [
            v[0] + v[1]
